...
to go if ticks >= srv_d [ stop ]
   tweet_action
   tweet_aging
   tick
end

;; executing execution operations
...
; tweet_aging (and cleanup): here, ip tweets that reached tweet lifespan "ls_twt" (from interface) are set-back to "NO" (= no ip-tweet)
to tweet_aging
  if ticks <= srv_d
  [
