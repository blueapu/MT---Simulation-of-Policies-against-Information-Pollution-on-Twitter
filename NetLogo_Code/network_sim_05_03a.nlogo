;;; network_sim_05_03 / 2021-03-10
;; DESCRIPTION: simulates spread of information pollution tweets in a network. Including countermeasures
;; Countermeasure: deleting % of accounts that issue a IP tweet "prc_ban"
;; VARIABLES:
; input "model_version" I/II: selection of different network models. "adjmat_large.txt" is a large model (3'032 nodes), derived from 10 candidates...
; input "model_version" II/II;) ... of 2019 Swiss elections. "adjmat_small.txt" is a small generic example (10 nodes). Both are ".txt-files" are adjecency matrices.
; input "no_ip_twt": number of initial information pollution tweets in entire network. From interface.
; input "p_ip_twt": probability of issuing a information tweet at all, given that a neighbor issued such a tweet (in percent). source: prob_input_large.txt or prob_input_small.txt
; input "p_ip_SUP": probability of a posting supporting tweet IF a tweet is posted (in percent). source: prob_input_large.txt or prob_input_small.txt
; input "p_ip_OPP": probability of posting an opposing tweet IF a tweet is posted (in percent). source: prob_input_large.txt or prob_input_small.txt
; input "p_ip_NTR": probability of posting a neutral tweet IF a tweet is posted (in percent). source: prob_input_large.txt or prob_input_small.txt
...
; input "prc_ban": is the % of accounts to be deleted that issue a IP tweet "ban_prc". from interface
; variable "post_ip_twt": indicator if turtle "has" an active information pollution tweet (removed after lifespan "ls_twt")
; variable "twt_time": time (= number of ticks) when the tweet was posted.
...
to readin-prob
 ...
 if model_version = "small"
  [
    ...
    [
    user-message "There is no file called prob_input_small.txt in current directory!" ; message, when no file "prob_input_01.txt" present ("else"-condition of first "ifelse")
    ]
  ]
 ...
 if model_version = "large"
  [
    ...
    [
    user-message "There is no file called prob_input_large.txt in current directory!" ; message, when no file "prob_input_01.txt" present ("else"-condition of first "ifelse")
    ]
  ]
...
to tweet_action
  ask turtles [
    ...
    let ip_cnt count friends with [ post_ip_twt = 1 ] ; defining ip_cnt as number of "friends" with tweeets ( post_ip_twt = "1") whose tweet is older than "twt_delay"
...
;; tweet countermeasure 03 "Account Ban"
; kills "prc_ban" percent of all tweets with an ip tweet
; the remaining probabilities (= two thirds of "p_ip_SUP") are evenly distributed to "p_ip_NTR" and "p_ip_OPP"
to tweet_countrmsr03
  let no_ip count turtles with [post_ip_twt = 1]
  let no_del ((no_ip * prc_ban)  / 100)
  ask n-of no_del turtles with [post_ip_twt = 1] [ die ]
end
