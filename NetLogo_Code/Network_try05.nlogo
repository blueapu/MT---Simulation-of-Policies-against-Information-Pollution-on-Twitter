;;; try05: Introduction of Change of Links "adjust-links"


;;; Open Tasks:
; defining a better measure for susceptibility
; checking why blue is more often (always) winning the upper hand
; finalizing "adjust-links"
...
turtles-own
[
  polview           ;; political view; 9 = fully "red"; 0 = fully "blue"
  suscept           ;; susceptibility to tweets; 0 = not susceptible, 1 = susceptible
  viewred           ;; number of red ads viewed
  viewblue           ;; number of blue ads viewed
]
...
to setup-nodes
  ...
  ask turtles [ set polview random 9 ]
  ask turtles [ set viewred 0 ]
  ask turtles [ set viewblue 0 ]
  ask turtles [ set suscept 1 ]
  ask turtles [ set label polview ]
 end

to setup-spatially-clustered-network
  let num-links (average-node-degree * number-of-nodes) / 2  ; defining total number of links
...
to setup-post
  ask n-of number-of-redposts turtles with [ polview > 6 ]  [
    set color red
  ] ; setting some of turtles with high values of "polview" "red"
  ask n-of number-of-blueposts turtles with [ polview < 2 ] [
    set color blue
  ] ; setting some of turtles with high values of "polview" "blue"
end
...
;; Action of viewing posts.
; dependent: color (= post, either "red" or "blue") of linked neighbors
; independent: "viewblue" and "viewred"
to view-post
  ask turtles
  [ let friends link-neighbors; defining "friends" as linked turtles
    set viewblue viewblue + count friends with [ color = blue ]
    set viewred viewred + count friends with [ color = red ]
  ]
end

;; Action of changing political view
; dependent: "viewred", "viewblue"; "suscept"
; independent: "polview"
to change-polview
  ask turtles [
    if (viewred > viewblue) and (suscept = 1)  [ set polview polview + 1 ]
    if polview > 9 [set polview 9]
    if (viewred < viewblue) and (suscept = 1) [ set polview polview - 1 ]
    if polview < 0 [ set polview  0]
    set label polview
  ]
end

;; Action of severing / Creating Links
; to adjust-links
;  ask turtles [
;    if
...
;; Action of posting a post (either becoming "red", "blue", "grey")
; dependent: polview
; independent: color of turtle
to post-ad
  ask turtles [
    if polview > 7 [ set color red ]
    if polview < 2 [ set color blue ]
    if (polview > 1) and (polview < 8) [ set color grey ]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"Red Posts" 1.0 0 -2674135 true "" "plot count turtles with [ color = red ]"
"Blue Posts" 1.0 0 -14070903 true "" "plot count turtles with [ color = blue ]"
"No Posts" 1.0 0 -7500403 true "" "plot count turtles with [ color = grey ]"
