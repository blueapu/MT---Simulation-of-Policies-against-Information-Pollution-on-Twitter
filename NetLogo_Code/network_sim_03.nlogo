;;; description of network_sim_03:
; input "adjmat_folwr_00.txt": is the network-topology (adjecency matrix)
; input "p_twt_00.txt": is a list that gives probabilites "p_ip_SUP", "p_ip_OPP", "p_ip_NTR" for each member of the network.
; input "no_ip_twt": number of information pollution tweets in entire network. From interface.
; input "p_ip_twt": probability of issuing a information tweet, given that a neighbor issued such a tweet (in percent). NOTE: this is a global value and the same for all members of the network. From interface.
...
to setup-network
   ...
   ifelse large_model ? [ nw:load-matrix "Data\\adjmat_folwr_00.txt" turtles links ;loading adjacency matrix  ]
                       [ nw:load-matrix "Data\\mt00.txt" turtles links ;loading adjacency matrix ]


   repeat 30 [  ; arranging nodes as if links were springs, "repeat" is part of it
    layout-spring turtles links 0.3 ((world-width / sqrt count turtles) / 2) 2
   ]
...
; information pollution setup. assigning (randomly) no_ip_twt tweets (neutral "NTR" and blue)
to setup-tweets
...
; tweet type probability setting for "NTR", "SUP" and "OPP" tweets [YYY - to find a way on how to get from list]
to setup-prob
  ask turtles [ set p_ip_NTR 10 ] ; setting indicator for information pollution tweets "0" (= no ip tweet)
  ask turtles [ set p_ip_SUP 10 ] ; setting indicator for information pollution tweets "0" (= no ip tweet)
  ask turtles [ set p_ip_OPP 80 ] ; setting indicator for information pollution tweets "0" (= no ip tweet)
end
...
; decision to tweet or not AND what type of tweet (neutral, supporting, opposing)
to tweet_decision
  ask turtles [

    let friends link-neighbors ; defining linked turtles as "friends"
    let ip_cnt count friends with [ post_ip_twt = 1 ] ; setting ip_cnt 1 of there is at least one ip-tweets among "friends"

    if ip_cnt > 0 and random 100 < p_ip_twt  ; checking if there are ip-tweets in surrounding, in p_ip_twt of all cases a tweet will be issued
    [
      set post_ip_twt 1 ; setting indicator that an ip tweet is posted
      let rdm_no random 100
    ...
    if twt_type = "NTR" [set color blue]
    if twt_type = "SUP" [set color red ]
    if twt_type = "OPP" [set color green ]
