;;; network_sim_05_01_test00
;; DESCRIPTION: simulates spread of information pollution tweets in a network. Including countermeasures
;; Countermeasure here: lowering probability issuing an supporting tweet by percentage "p_warn" (p_warn = 100 means no probability)
;; VARIABLES
; input "model_version" I/II: selection of different network models. "adjmat_folwr_00.txt" is a large model (~3'300 nodes), derived from 10 candidates...
; input "model_version" II/II;) ... of 2019 Swiss elections. "mt00.txt" is a small generic example (10 nodes). Both are ".txt-files" are adjecency matrices.
; input "no_ip_twt": number of initial information pollution tweets in entire network. From interface.
; input "p_ip_twt": probability of issuing a information tweet at all, given that a neighbor issued such a tweet (in percent). NOTE: this is a global value and the same for all members of the network. From interface.
; input "p_ip_SUP": probability of a posting supporting tweet (supporting the doubious claim) IF a tweet is posted (in percent).
; input "p_ip_OPP": probability of posting an opposing tweet (opposing the doubious claim) IF a tweet is posted (in percent).
; input "p_ip_NTR": probability of posting a neutral tweet (neutral towards the doubious claim) IF a tweet is posted (in percent).
; input "ls_twt": is the lifespan of a tweet in ticks. After this number of ticks, the tweet is removed from consideration (appears as the account did not tweet at all). From interface.
; input "p_warn": is the percentage by what the probability of issuing an ip tweet is lowered. From interface
; variable "post_ip_twt": indicator if turtle "has" an active information pollution tweet (removed after lifespan "ls_twt")
; variable "tm_twt": counts the time (= number of ticks) since a tweet was posted
; variable "twt_type": indicates type of tweet. Either supporting "SUP", opposing "OPP", neutral "NTR" or no tweet "NA"
...
to setup-network
  ...
  if model_version = "large" [ nw:load-matrix "Data\\adjmat_large.txt" turtles links ] ; loading adjacency matrix
  if model_version = "small" [ nw:load-matrix "Data\\adjmat_small.txt" turtles links ] ; loading adjacency matrix
  if model_version = "test" [ nw:load-matrix "Data\\test_adj_matx00.txt" turtles links ] ; loading adjacency matrix
...
; reading-in probabilities for each turtle from file "prob_input_small.txt" or "prob_input_large.txt"
; NOTE: file "prob_input_XXX.txt" is converted in one single vector "prob_lst" (e.g. [65 10 25 80 20 0...]).
; .. .Each triplet belongs to one turtle e.g. turtle 0: 65 10 25, turtle 1: 80 20 0 and so on.
; ... Every first entry is "probability NTR", every second entry is "probability OPP" and every third entry is "probability SUP"
; ... Refer to chunk "setup-prob" below.

to readin-prob

 if model_version = "small"
  [
    ifelse ( file-exists? "Data\\prob_input_small.txt" ) ; check, if file "prob_input_01.txt" exists in specified folder
    [
     set prob_lst [] ; preparing a list for probabilities

    file-open "Data\\prob_input_small.txt" ; opening the data
      ...
      user-message "Input file prob_input_small.txt sucessfully loaded!" ; message when sucessfully loaded
      file-close ; closing file
    ]

    [
    user-message "There is no file called prob_input_small.txt in directory Data!" ; message, when no file "prob_input_small.txt" present ("else"-condition of first "ifelse")
    ]
  ]
 ...
 if model_version = "large"
  [
    ifelse ( file-exists? "Data\\prob_input_large.txt" ) ; check, if file "prob_input_01.txt" exists in specified folder
    [
     set prob_lst [] ; preparing a list for probabilities

    file-open "Data\\prob_input_large.txt" ; opening the data
      ...
      user-message "Input file prob_input_large.txt sucessfully loaded!" ; message when sucessfully loaded
      file-close ; closing file
    ]

    [
    user-message "There is no file called prob_input_large.txt in directory Data!" ; message, when no file "prob_input_small.txt" present ("else"-condition of first "ifelse")
    ]
  ]



 if model_version = "test"
  [
    ifelse ( file-exists? "Data\\prob_input_test00.txt" ) ; check, if file "prob_input_01.txt" exists in specified folder
    [
     set prob_lst [] ; preparing a list for probabilities

    file-open "Data\\prob_input_test00.txt" ; opening the data
      ...
      user-message "Input file prob_input_test00.txt sucessfully loaded!" ; message when sucessfully loaded
      file-close ; closing file
    ]

    [
    user-message "There is no file called prob_input_test00.txt in directory Data!" ; message, when no file "prob_input_test00.txt" present ("else"-condition of first "ifelse")
    ]
  ]

end

; assigning read-in probabilites "prob_lst" to turtles...
; ...refer to chunk "readin-prob" above for information on the
to setup-prob
  ask turtles [
    set p_ip_NTR item ((who * 3) + 0) prob_lst ; every first (+ 0) entry is probability "NTR"
    set p_ip_OPP item ((who * 3) + 1) prob_lst ; every second entry (+ 1) is probability "OPP"
    set p_ip_SUP item ((who * 3) + 2) prob_lst ; every third entry (+ 2) is probability "SUP"
  ]
end
...
;; tweet countermeasure 01 "Warning"
;  the probability of supporting tweets "p_ip_SUP" are lowered by "p_warn" percent. (p_warn = 100 means all)
; only applies at the first tick
; the remaining probabilities are evenly distributed to "p_ip_NTR" and "p_ip_OPP"
to tweet_countrmsr01
  if ticks = 1 [
    ask turtles [
      let p_warneff ((p_ip_SUP / 100) * p_warn)
      set p_ip_SUP (p_ip_SUP - p_warneff)
      set p_ip_NTR (p_ip_NTR + (p_warneff / 2))
      set p_ip_OPP (p_ip_OPP + (p_warneff / 2))
    ]
  ]
end
...
to tweet_action
  ask turtles [
    ...
    [
      ...
      set tm_twt 1 ; setting "tweet timer" to "1"
