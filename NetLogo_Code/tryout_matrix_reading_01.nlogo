...
; input "adjmat_folwr_01.txt": is the network-topology (adjecency matrix)
...
; input "p_ip_twt": probability of issuing a information tweet, given that a neighbor issued such a tweet. NOTE: this is a global value and the same for all members of the network. From interface.
; input "p_ip_SUP": probability of a posting supporting tweet (supporting the doubious claim) IF a tweet is posted.
; input "p_ip_OPP": probability of posting an opposing tweet (opposing the doubious claim) IF a tweet is posted.
; input "p_ip_NTR": probability of posting a neutral tweet (neutras towards the doubious claim) IF a tweet is posted.
; inputs "p_ip_SUP", "p_ip_OPP", "p_ip_NTR": the sum of all these probabilies is 1.0
; inputs "p_ip_SUP", "p_ip_OPP", "p_ip_NTR": the values are dependent on
...
[
  ...
  p_ip_SUP ; probability of a posting supporting tweet
  p_ip_OPP ; probability of a posting opposing tweet
  p_ip_NTR ; probability of a posting neutral tweet
]
...
to setup-network
   ...
   nw:load-matrix "Data\\adjmat_folwr_00.txt" turtles links ;loading adjacency matrix
