...
to setup-network
  ...
  if model_version = "test" [ nw:load-matrix "Data\\test_adj_matx01.txt" turtles links ] ; loading adjacency matrix
...
to setup-prob
  ask turtles [
    set p_ip_NO item ((who * 3) + 0) prob_lst ; every first (+ 0) entry is probability "NO"
    set p_ip_NTR item ((who * 3) + 1) prob_lst ; every second entry (+ 1) is probability "NTR"
    set p_ip_OPP item ((who * 3) + 2) prob_lst ; every third entry (+ 2) is probability "OPP"
    set p_ip_SUP item ((who * 3) + 3) prob_lst ; every third entry (+ 3) is probability "SUP"
  ]
end
