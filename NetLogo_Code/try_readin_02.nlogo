...
; ... in one single vector "prob_lst" (e.g. [0.1 0.3 0.7 0.0 0.2 0.8...]). Each triple belongs to one turtle...
; ... e.g. turtle 1: 0.1 0.3 0.7, turtle 2: 0.0 0.2 0.8 and so on. Every first entry is "probability NTR"...
; ...every second entry is "probability OPP" and every third entry is "probability SUP". Refer to chunk "setup-prob" below.
to readin-prob
...
to setup-prob
  ask turtles [
    ...
    set p_ip_OPP item ((who * 3) + 1) prob_lst ; every second entry (+ 1) ist probability OPP
    set p_ip_SUP item ((who * 3) + 2) prob_lst ; every third entry (+ 2) ist probability SUP
  ]
end
