;;; network_sim_05_01 / 2021-02-10
;; DESCRIPTION: simulates spread of information pollution tweets in a network. Including countermeasures
;; Countermeasure here: Issuing a warning lowering the probability issuing a supporting tweet variable: "p_ip_SUP"
;; VARIABLES
...
; if warning has been selected (interface), the probability of supporting tweets are lowered to a third of it's original probability
; the remaining probabilities (= two thirds of "p_ip_SUP") are evenly distributed to "p_ip_NTR" and "p_ip_OPP"
to tweet_countrmsr01
  if warning = "YES" and ticks = 1 [
    ask turtles [
      set p_ip_NTR ((p_ip_SUP / 3) + p_ip_NTR)
      set p_ip_OPP ((p_ip_SUP / 3) + p_ip_OPP)
      set p_ip_SUP (p_ip_SUP * (1 / 3))
    ]
  ]
end
