...
;; 4) delay till tweet is shown "dly_twt"
...
[
  ...
  twt_time ; time stamp when tweet is posted
...
if ntwrk = "real" [
  ...
  ; option "less dense network" with an average of 15 links/turtle (ntwrk = "less_15")
if ntwrk = "less_15" [
     create-turtles 858
    ...
    ; option "less dense network" with an average of 45 links/turtle (ntwrk = "more_45")
if ntwrk = "more_45" [
     create-turtles 858
...
to readin-prob
    ...
    file-open "Data\\prob_input_test03.txt" ; open file that contains predicted probabilities
...
to go
   ...
   if count turtles with [ post_ip_twt = 1 ] <= 0 [ ; stopping & reporting after no of ticksd
                      output-print sum_newip
                      output-print count turtles
                      output-print ticks
                      stop ]
    ...
    [
      ...
      let rdm_no random 101 ;
        ...
        if rdm_no > p_ip_NO and rdm_no <= (p_ip_NO + p_ip_NTR) [set twt_type "NTR"  ; in "p_ip_NTR" percent of all cases, the following applies
                                                                ...
                                                                set post_ip_twt 1
                                                                set tm_twt 0
                                                                set twt_time ticks] ; setting time stamp
