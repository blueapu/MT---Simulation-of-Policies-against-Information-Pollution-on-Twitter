...
to setup-network
  ...
  if model_version = "large" [ nw:load-matrix "Data\\adjmat_folwr_00.txt" turtles links ] ; loading adjacency matrix
  if model_version = "small" [ nw:load-matrix "Data\\mt00.txt" turtles links ] ; loading adjacency matrix
...
; tweet type probability setting for "NTR", "SUP" and "OPP" tweets
; NOTE: [YYY - to find a way on how to get from list]
to setup-prob
  ask turtles [ set p_ip_NTR 20 ] ; setting probabilty for neutral tweets to 20%
  ask turtles [ set p_ip_SUP 10 ] ; setting probabilty for supporting tweets to 10%
  ask turtles [ set p_ip_OPP 70 ] ; setting probabilty for opposing tweets to 70%
end
