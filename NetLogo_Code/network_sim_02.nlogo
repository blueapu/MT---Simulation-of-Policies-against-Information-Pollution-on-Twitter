;;; description of network_sim_02: introduction of amount of time a tweet is read by followers ("tweet_latency"), after this time it is not read anymore.
; input network-topology: retreived from 10 randomly selected candidates of 2019 elections (10 canditates and all their followers)
; input number of supporting tweets "no_of_supporting": from interface
; input number of opposing tweets "no_of_opposing": from interface
; input amount of time a tweet is read by followers "tweet_latency": from interface
; method of decision: balance of neighbor's opinion (e.g: if more supporting, then support / if equal number of supporting and opposing, then neutral)
...
[
  ...
  tweet_date ; date (no. of ticks when this tweet was tweeted)
]
...
to setup-network
   ...
   nw:load-matrix "Data\\adjmat_folwr_00.txt" turtles links ;loading adjacency matrix derived from (10 canditates and all their followers)
  ...
  ask turtles [ set tweet_date 0 ]
end

; information pollution setup
to setup-tweets

   ask n-of no_of_supporting turtles [ set post_pollut_cont "S" ]  ; assigning (randomly) number of supporting tweets
   ask n-of no_of_opposing turtles with [ post_pollut_cont = "N"] [ set post_pollut_cont "C" ] ; assigning (randomly) number of contradicting tweets to remaining tweets

    ask turtles [
    if post_pollut_cont = "S" [ set color green ]
    if post_pollut_cont = "C" [ set color red ]
  ]
end
...
to tweet_reading
  ask turtles [
    ...
    set view_pollut_sup count friends with [ post_pollut_cont = "S" and (ticks - tweet_date) < tweet_latency ]
    set view_pollut_opp count friends with [ post_pollut_cont = "C" and (ticks - tweet_date) < tweet_latency ]
  ]
end
...
to tweet_action
  ask turtles [
    if view_pollut_opp = "C" [set tweet_date tweet_date + 1 ]
    if view_pollut_opp = "S" [set tweet_date tweet_date + 1 ]
    ...
    if post_pollut_cont = "N" [ set color grey ]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
575
456
Supporting, Opposing and Neutral Tweets
Time [days]
Number of Tweets [--]
0.0
10.0
...
"" ""
PENS
"Neutral Tweets" 1.0 0 -9276814 true "" "plot count turtles with [ color = grey ]"
"Opposing Tweets" 1.0 0 -5298144 true "" "plot count turtles with [ color = red ]"
"Supporting Tweets" 1.0 0 -14439633 true "" "plot count turtles with [ color = green ]"
...
531
Turtles with "S"
count turtles with [ post_pollut_cont = \"S\" ]
17
1
11
...
593
Turles with "C"
count turtles with [ post_pollut_cont = \"C\" ]
17
1
11
...
656
Turtles with "N"
count turtles with [ post_pollut_cont = \"N\" ]
17
1
11
